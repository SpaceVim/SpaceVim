...
function! SpaceVim#plugins#defind_hooks(bundle) abort
  if g:spacevim_plugin_manager ==# 'neobundle'
    ...
    func! s:hooks.on_source(bundle) abort
      call SpaceVim#plugins#source_rc('plugins/' . split(a:bundle['name'],'\.')[0] . '.vim')
    endf
  elseif g:spacevim_plugin_manager ==# 'dein'
    call dein#config(g:dein#name, {
          \ 'hook_source' : "call SpaceVim#plugins#source_rc('plugins/" . split(g:dein#name,'\.')[0] . ".vim')"
          \ })
  endif
endfunction
...
function! SpaceVim#plugins#loadPluginBefore(plugin) abort
  if matchend(a:plugin, "\\.vim") == len(a:plugin)
    call SpaceVim#plugins#source_rc('plugins_before/' . a:plugin)
  elseif matchend(a:plugin, "\\.nvim") == len(a:plugin)
    call SpaceVim#plugins#source_rc('plugins_before/' . a:plugin[:-6] . '.vim')
  else
    call SpaceVim#plugins#source_rc('plugins_before/' . a:plugin . '.vim')
  endif
endfunction
